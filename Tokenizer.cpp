...
//Prints a list of found operators, in the order that they appeared
void Tokenizer::printOperators(){
	vector<string>::iterator it;
...
void Tokenizer::findOperators(string str){
	string op;
	...
	for(unsigned int i = 0; i < str.length(); ++i){
		switch(str[i]){
			...
			case '*':
			case '!':
				//Simple operator found
				op.push_back(str[i]);
				if(!isOperatorFound(op)){
					//Add to operator vector
			...
			case '&':
			case '|':
				//Compound operator found
				if(str[i] == str[i+1]){
					//Store found operator
					op.push_back(str[i]);
			...
			case '=':
				...
				//Not a compound operator
					...
					//Check for delimiter at the end of a statement
					...
					//Check for semicolon in substr
					string substr = str.substr(i, k);
					//No semicolon found
					if(substr.find(';') == string::npos){
						errors.push_back("Missing ';' at end of statement");
					}
				}
...
//Checks if an operator has been previously found
bool Tokenizer::isOperatorFound(string op){
	vector<string>::iterator it;
	
	for(it = operators.begin(); it != operators.end(); ++it){
		//Operator has been previously found
		if(it->compare(op) == 0) return true;
	}
	
	//Operator has not been previously found
	return false;
}
